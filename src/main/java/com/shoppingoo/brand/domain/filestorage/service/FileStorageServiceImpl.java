...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    @Override
    public Mono<String> saveImageFile(MultipartFile file) throws IOException {
        // AWS 자격 증명 설정
        ...
        file.transferTo(localFile); // MultipartFile을 로컬 파일로 저장

        // S3 버킷에 파일 업로드
        s3Client.putObject(new PutObjectRequest(bucketName, fileName, localFile));

        // 업로드된 파일 URL 반환
        return Mono.fromCallable(() -> s3Client.getUrl(bucketName, fileName).toString());
    }
}
